...
import { fadeUp, fadeUpContent, staggerContainer, rowItem, viewport, viewportEarly } from '@/lib/animations'
...
export default function Cleaning() {
  return (
    <section id="cleaning" className="bg-cream relative overflow-hidden pb-32 pt-20 lg:pb-40 lg:pt-28">

      {/* Ghost text — section-level backdrop */}
      <span
        aria-hidden="true"
        className="text-teal-dark/6 pointer-events-none absolute top-0 left-0 select-none text-[10rem] font-bold leading-none tracking-tighter lg:text-[20rem]"
      >
        CLEAN
      </span>
      ...
      <div className="relative z-1 mx-auto max-w-6xl px-6">
        ...
        >
          ...
          <div className="divide-teal-dark/10 divide-y">
            {cleaningTypes.map((item) => (
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className="text-teal-dark text-xl font-semibold sm:text-2xl">{item.name}</h3>
                  <p className="text-neutral-mid mt-2 max-w-2xl text-base leading-7">
                    {item.description}
                  </p>
                </div>
              </motion.div>
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={viewport}
          className="border-teal-dark/10 mt-16 grid grid-cols-1 gap-8 border-t pt-14 lg:grid-cols-2 lg:gap-16"
        >
          {/* Left */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-gold">Day Porter</p>
            <div className="mt-3 h-0.5 w-10 bg-gold" aria-hidden="true" />
            <h3 className="text-teal-dark mt-4 text-2xl font-semibold">
              A dedicated presence during business hours.
            </h3>
            <p className="text-neutral-mid mt-4 text-base leading-7">
          ...
          <ul className="space-y-4 self-center">
            {dayPorterPoints.map((point) => (
              <li key={point} className="flex items-baseline gap-3">
                ...
                <span className="text-neutral-mid text-base leading-7">{point}</span>
              </li>
            ))}
          </ul>
        </motion.div>
